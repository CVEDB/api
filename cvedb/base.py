...
class CvedbApiError(Exception):
    def __init__(self, http_status, data):
        super(CvedbApiError, self).__init__(data)
        self.http_status = http_status


class CvedbApiMeta(type):
...
class PersistentCookieJar(cookiejar.MozillaCookieJar, cookies.RequestsCookieJar):
    def __init__(self, filename=None, *args, **kwargs):
        if filename is None:
            cache_dir = user_cache_dir("Cvedb", "Cvedb.Inc")
...
class CvedbApiBase(with_metaclass(CvedbApiMeta)):
    # Retry status codes
    ...
    def __init__(self, api_key, proxies=None, persistent=True, server_url="https://cvedb.github.io"):
        """
        Create VScanner API object.

        :param api_key:
            string with Cvedb API key. You can obtain one from the https://cvedb.github.io
        :param proxies:
            dict for proxy supporting. Example: {"https": "myproxy.com:3128"}
        """
        if not api_key:
            raise ValueError(
                "API key must be provided. You can obtain one for free at https://cvedb.github.io"
            )
        ...
        session.headers.update(
            {
                ...
                "User-Agent": "Cvedb Python API %s" % __version__,
            }
        )
        if persistent:
    ...
    def adapt_response(response, method, expected_result):
        if expected_result == "json":
            ...
            if response.status_code >= 400:
                raise CvedbApiError(response.status_code, result)
            if isinstance(result, dict) and "data" in result:
                data = result["data"]
                if isinstance(data, dict) and data.get("error"):
                    raise CvedbApiError(response.status_code, data)
                return data
            return result
    ...
    def _update_ratelimit(bucket, response):
        ...
        limit = headers.get("X-Cvedb-Ratelimit-Reqlimit")
        if limit:
            try:
