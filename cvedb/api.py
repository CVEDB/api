...
class Cvedb(object):
    ...
    Cvedb backend dynamic blocking cache is changing this value depending on the server load and client license.
    ...
    It's used for better session handling at Cvedb server side and really helps a lot not to overcreate sessions.
    But if you feels not comfortable - you can just turn it off at the init state setting "persistent = False"
    """

    # Cvedb hostname is setting up individually. For on-premise installations it must be replaced with local one.

    cvedb_hostname = 'https://cvedb.guthub.io'
    ...
    # Default URL's for the Cvedb API
    api_endpoints = {
    ...
    def __init__(self, api_key, proxies=None, persistent=True):
        ...
        :param api_key: string with Cvedb API key. You can obtain one from the https://cvedb.guthub.io
        :param persistent: Boolean. Regulates cookie storage policy. If set to true - will save down session cookie for reuse.
        """
        self.cvedb_urls = dict((key, self.cvedb_hostname + self.api_endpoints.get(key)) for key in self.api_endpoints)
        ...
        adapter = HTTPAdapter(
            ...
            pool_maxsize=100,
            max_retries=retries)
        self.__opener.mount(self.cvedb_hostname, adapter)
        self.__opener.headers.update({'Connection': 'Keep-Alive'})
        #
        self.__opener.headers.update({'User-Agent': 'Cvedb Python API %s' % api_version})
        ...
        if not api_key:
            raise ValueError("API key must be provided. You can obtain one for free at https://cvedb.guthub.io")
        ...
        if api_key and not self.__validKey(api_key):
            raise ValueError("Wrong Cvedb API key or API key are expired. Please, follow https://cvedb.guthub.io to obtain correct one.")
    ...
    def __adapt_response_content(self, response):
        ...
        if response.status_code == 402 and response.json()['data']['errorCode'] == 9000:
            raise AssertionError("Bad or no API key provided. Please, obtain correct one registering at https://cvedb.guthub.io")
    ...
    def __update_ratelimit(self, api_short_name, response):
        ...
        :param api_short_name: API shortened name from the __cvedb_urls
        :param response: requests lib response
        :return: True/False
        """
        headers = response.headers

        ratelimit = headers.get('X-Cvedb-Ratelimit-Reqlimit')
        current_rate = headers.get('X-Cvedb-Ratelimit-Rate')
    ...
    def cvedb_get_request(self, cvedb_url_key, json_parameters):
        """
        Tech wrapper for the unified

        :param cvedb_url_key: Key for the self.cvedb_urls dict
        ...
        response = self.__opener.get(self.cvedb_urls[cvedb_url_key], params=json_parameters)
        # Update rate limits
        self.__update_ratelimit(cvedb_url_key, response)
        return self.__adapt_response_content(response)

    @rate_limited(api_rate_limits)
    def cvedb_post_request(self, cvedb_url_key, json_parameters):
        """
        Tech wrapper for the unified
        ...
        if self.__api_key:
            json_parameters['apiKey'] = self.__api_key
        response = self.__opener.post(self.cvedb_urls[cvedb_url_key], json=json_parameters)
        # Update rate limits
        self.__update_ratelimit(cvedb_url_key, response)
        return self.__adapt_response_content(response)
    ...
    def __validKey(self, api_key):
        ...
        :param api_key: Cvedb API Key
        :return: True/False
        """
        if not isinstance(api_key, string_types):
            raise TypeError("api_key expected to be a string")

        return self.cvedb_post_request('apiKey', {'keyID':api_key}).get('valid')
    ...
    def __archive(self, type, datefrom, dateto):
        ...
        return self.cvedb_get_request('archive', {'type':type, 'datefrom':'' or datefrom, 'dateto':'' or dateto})
    ...
    def __distributive(self, os, version):
        ...
        return self.cvedb_get_request('distributive', {'os':os, 'version':version})
    ...
    def __search(self, query, skip, size, fields):
        ...
        if not isinstance(skip, int) or not 0 <= skip <= 9999:
            raise TypeError(
                "Skip expected to be an int in range 0-9999, Cvedb "
        ...
        if not isinstance(size, int) or not 0 <= size <= 100:
            raise TypeError(
                ...
                "Cvedb has a hard limit on max response size equal to 100")
        return self.cvedb_post_request('search', {"query":query, 'skip': skip or 0, 'size': size or 0, 'fields': fields or []})
    ...
    def __id(self, identificator, references, fields):
        ...
        return self.cvedb_post_request('id', search_request)
    ...
    def __software_audit(self, packages, os, os_version):
        ...
        return self.cvedb_post_request('softwareAudit', {"os":os, 'osVersion':os_version, 'packages':packages})
    ...
    def __audit(self, os, os_version, package):
        ...
        if not isinstance(package, (list, set)):
            raise TypeError("Package expected to be a list or set")
        return self.cvedb_post_request('audit', {"os":os, 'version':os_version, 'package':package})
    ...
    def __kbAudit(self, os, kb_list):
        ...
        return self.cvedb_post_request('kbAudit', {"os":os, 'kbList':kb_list})
    ...
    def __burpSoftware(self, software, version, type, maxVulnerabilities, exactmatch=False):
        ...
        return self.cvedb_post_request('software', {"software":software,
                                                      'version':version,
                                                      'type':type,
    ...
    def __suggest(self, type, field_name):
        ...
        return self.cvedb_post_request('suggest', {"type":type, 'fieldName':field_name})
    ...
    def __ai_score(self, text):
        ...
        return self.cvedb_post_request('ai', {"text":text})
    ...
    def __autocomplete(self, query):
        ...
        return self.cvedb_post_request('autocomplete', {"query":query})

    def search(self, query, limit=100, offset=0, fields=None):
        """
        Search Cvedb database for the abstract query

        Retrieves up to 10000 records.

        :param query: Abstract Cvedb query. See https://cvedb.guthub.io/help for the details.
        :param limit: a.k.a. search size. Default is 100 records.
        :param offset: Skip this amount of documents. 9999 is Cvedb' absolute maximum.
    ...
    def searchPage(self, query, pageSize = 20, offset=0, fields=None):
        """
        Search Cvedb database for the abstract query, page mode
        ...
        if not isinstance(results, dict):
                raise AssertionError(
                    "Asserted result failed. No JSON returned from Cvedb.\n"
    ...
    def searchExploit(self, query, lookup_fields=None, limit=100, offset=0, fields=None):
        """
        Search Cvedb database for the exploits
    ...
    def searchExploitPage(self, query, lookup_fields=None, pageSize=20, offset=0, fields=None):
        """
        Search Cvedb database for the exploits, page mode
    ...
    def winaudit(self, os, os_version, kb_list, software):
        ...
        return self.cvedb_post_request('winaudit',
    ...
    def collections(self):
        """
        Get list of the Cvedb collection type names
    ...
    def aiScore(self, text):
        """
        Score free text upon Cvedb AI network
    ...
    def rules(self):
        ...
        """
        return self.cvedb_get_request('rules', {}).get('rules', {})
    ...
    def archive(self, collection, start_date='', end_date=''):
        ...
        with ZipFile(BytesIO(zipped_json)) as zip_file:
            if len(zip_file.namelist()) > 1:
                raise Exception("Unexpected file count in Cvedb ZIP archive")
    ...
    def autocomplete(self, query):
        """
        Ask Cvedb for possible suggestions to complete your query

        :param query: Cvedb Search query, Lucene syntax
        :return: Float score
        """
